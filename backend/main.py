...
Native (iOS/Android) push notifications: the Capacitor mobile app registers
an FCM device token instead of a browser push subscription -- stored
separately in `device_push_tokens` and managed via /push/subscribe-device
and /push/unsubscribe-device. Every place this server sends a Web Push
notification, it also sends to any registered device tokens for that same
email, through Firebase Cloud Messaging (which delivers to both Android
directly and iOS via APNs under the hood, so one integration covers both
platforms). Like VAPID below, this is entirely optional -- if
FIREBASE_SERVICE_ACCOUNT_JSON isn't set, native push sends are silently
skipped rather than erroring, exactly like unconfigured Web Push.
...
this file):
  ...
  FIREBASE_SERVICE_ACCOUNT_JSON   optional -- the *entire* service account
                             JSON key file (Firebase console -> Project
                             Settings -> Service Accounts -> Generate new
                             private key), pasted as one line. Only needed
                             for native mobile push; web push and everything
...
def _init_firebase() -> bool:
    """Parses FIREBASE_SERVICE_ACCOUNT_JSON and initializes the Firebase
    Admin SDK, if that env var is set. Returns whether native push is
    usable. Failures are logged and treated the same as "not configured"
    (native push sends silently skip) rather than crashing the server --
    this integration is optional, unlike Supabase."""
    ...
    try:
        ...
        logger.error(
            f"FIREBASE_SERVICE_ACCOUNT_JSON is set but could not be parsed/used "
            f"-- native push notifications will silently fail until this is "
            f"fixed. Paste the *entire* service account JSON file as one line. "
            f"Underlying error: {e}"
        )
        return False


_native_push_configured: bool = _init_firebase()
...
def health_check():
    ...
    return {
        ...
        "native_push_configured": _native_push_configured,
    }
...
# --------------------------- Native (mobile) device tokens ---------------------------
# Mirrors the push_subscriptions functions above exactly, just against
# device_push_tokens and FCM instead of push_subscriptions and Web Push.

async def upsert_device_token(token: str, email: str, platform: str) -> None:
    payload = {"token": token, "email": email, "platform": platform}
    async with httpx.AsyncClient(timeout=5.0) as client:
        resp = await client.post(
            f"{SUPABASE_URL}/rest/v1/device_push_tokens",
            json=payload,
            headers=_service_headers({"Prefer": "resolution=merge-duplicates,return=minimal"}),
        )
    resp.raise_for_status()
...
async def delete_device_token(token: str, email: Optional[str] = None) -> None:
...
async def get_device_tokens_for_email(email: str) -> List[dict]:
    async with httpx.AsyncClient(timeout=5.0) as client:
        resp = await client.get(
            f"{SUPABASE_URL}/rest/v1/device_push_tokens",
            params={"email": f"eq.{email}", "select": "*"},
...
def _send_one_native_push(device_token: str, payload: dict) -> Optional[str]:
    """Blocking call (firebase-admin uses requests under the hood), meant to
    be run via asyncio.to_thread. Returns the device token if it turned out
    to be expired/invalid/unregistered and should be deleted, else None.
    All notification fields are also mirrored into `data` (string values
    only, as FCM requires) so the app can read them the same way regardless
    ...
    try:
        ...
        return device_token  # token no longer registered on FCM's side
    except Exception as e:
        logger.warning(f"Native push send failed: {e}")
    return None


async def _send_native_push_to_email(email: str, payload: dict) -> None:
    """Best-effort fan-out to every device this email has registered.
    Called alongside (not instead of) the Web Push sends in each
    send_*_push function below -- a person might have both a browser
    subscription and the mobile app, and should get both."""
    if not _native_push_configured:
        return
    try:
        rows = await get_device_tokens_for_email(email)
    except httpx.HTTPError as e:
        logger.warning(f"Could not fetch device tokens for {email}: {e}")
        return
    for row in rows:
        expired_token = await asyncio.to_thread(_send_one_native_push, row["token"], payload)
        if expired_token:
            logger.info(f"Device token for {email} is expired/invalid -- removing it.")
            try:
                await delete_device_token(expired_token)
            except httpx.HTTPError:
                pass
        else:
            logger.info(f"Sent native push to {email}")
...
async def send_message_push(to_email: str, from_display: str, conversation_id: str, preview: str) -> None:
    ...
    if _vapid_key_file and VAPID_PUBLIC_KEY and VAPID_CONTACT_EMAIL:
        try:
    ...
    await _send_native_push_to_email(to_email, payload)
...
async def send_incoming_call_push(to_emails: List[str], room_code: str, caller_email: str) -> None:
    ...
    web_push_ready = bool(_vapid_key_file and VAPID_PUBLIC_KEY and VAPID_CONTACT_EMAIL)
    for email in to_emails:
        ...
        await _send_native_push_to_email(email, payload)
...
class DeviceTokenSubscribeBody(BaseModel):
    token: str  # Supabase session token, same auth pattern as PushSubscribeBody
    device_token: str  # the FCM registration token for this install
    platform: str  # "ios" | "android"


class DeviceTokenUnsubscribeBody(BaseModel):
    token: str
    device_token: str


@app.post("/push/subscribe-device")
async def push_subscribe_device(body: DeviceTokenSubscribeBody):
    user = await verify_supabase_token(body.token)
    if user is None:
        return {"error": "invalid session"}, 401
    ...
    email = (user.get("email") or "").lower()
    await upsert_device_token(body.device_token, email, body.platform)
    return {"status": "subscribed"}


@app.post("/push/unsubscribe-device")
async def push_unsubscribe_device(body: DeviceTokenUnsubscribeBody):
    user = await verify_supabase_token(body.token)
    if user is None:
        return {"error": "invalid session"}, 401
    email = (user.get("email") or "").lower()
    await delete_device_token(body.device_token, email=email)
    return {"status": "unsubscribed"}
