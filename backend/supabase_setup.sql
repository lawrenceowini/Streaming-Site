...
-- ---------------------------------------------------------------------------
-- Mobile app (Capacitor): native push tokens. Same access model as
-- push_subscriptions above and for the same reason -- RLS on, no policies,
-- so only the signaling server (via the service_role key) can read or write
-- it, through the new /push/subscribe-device and /push/unsubscribe-device
-- endpoints. Kept as a separate table rather than folded into
-- push_subscriptions since the two are different shapes end to end: Web
-- Push sends a JSON subscription object through pywebpush, while native
-- push sends a single FCM device token through firebase-admin -- different
-- delivery mechanism, different payload, different expiry behavior.
-- ---------------------------------------------------------------------------
